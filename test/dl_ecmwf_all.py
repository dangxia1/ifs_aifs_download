"""Download single-level + pressure-level data, merge into one file."""
import os
import tempfile
from ecmwf.opendata import Client

TARGET = os.path.join(os.path.dirname(__file__), "data_2026-05-02_step0_all.grib2")
...
# -- 1. Try single call with all params --
print("Trying single retrieve() with all params ...")
client.retrieve(
    date="2026-05-02", time=0, type="fc", step=0,
    param=SINGLE_PARAMS + LEVEL_PARAMS, levelist=LEVELS, target=TARGET,
)

# Check if single-level params were included
import eccodes
...
if single_missing:
    ...
    tmp1 = os.path.join(tempfile.gettempdir(), "tmp_single.grib2")
    tmp2 = os.path.join(tempfile.gettempdir(), "tmp_levels.grib2")
