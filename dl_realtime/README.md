...
验证：viewer.html 头部注释含 `v2.6` = 代码是新版（页脚文字已按老师要求删除）；深色科学仪器风（单一琥珀强调色、图淡入、加载骨架屏、大字号）。v2.2 另含：Tab/区域按钮可点击、图片按原始比例放大展示；v2.3 另含：播放动画预载后续帧+保留上一帧（大图加载慢不再"隔两张黑一下"）、step 防御回退+骨架/提示覆盖图片区（杜绝 stepundefined 黑框）、删除时序图下方解释文字；v2.4：图片加载失败静默处理；v2.5：无数据时完全静默；v2.6：**真正根因**——"未找到图片数据目录"是 placeholder 元素静态文字，`.placeholder{display:flex}` 覆盖了 UA 的 `[hidden]{display:none}`，该文字从首版起就永远悬浮在图片正中，现元素已整个删除（HTML+CSS+JS）。⚠️ 教训：给带 `hidden` 的元素设 `display` 属性时，必须同时写 `.xxx[hidden]{display:none}`，否则 hidden 失效。
