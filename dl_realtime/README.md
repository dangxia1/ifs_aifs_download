...
验证：viewer.html 头部注释含 `v2.3` = 代码是新版（页脚文字已按老师要求删除）；深色科学仪器风（单一琥珀强调色、图淡入、加载骨架屏、大字号）。v2.2 另含：Tab/区域按钮可点击、图片按原始比例放大展示；v2.3 另含：播放动画预载后续帧+保留上一帧（大图加载慢不再"隔两张黑一下"）、step 防御回退+骨架/提示覆盖图片区（杜绝 stepundefined 黑框）、删除时序图下方解释文字。
